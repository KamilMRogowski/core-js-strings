...
function removeFirstOccurrences(str, value) {
  ...
  const part2 = str.slice(index + value.length, str.length);
...
function isPalindrome(str) {
  const normal = str.toLowerCase().replaceAll(/[^a-z]/g, '');
...
function invertCase(str) {
  ...
  const upper = /[A-Z]/g;
  const letters = str.split('');
  const invertedLetters = letters.map((letter) => {
...
function getCardId(value) {
  ...
  // Calculate index using the positions of rank and suit in the initial deck
  const rankIndex = ranks.indexOf(rank);
  const suitIndex = suits.indexOf(suit);
  const cardIndex = rankIndex + suitIndex * ranks.length;
